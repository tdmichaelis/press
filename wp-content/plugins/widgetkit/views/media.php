<div class="uk-modal-dialog uk-modal-dialog-large" data-media-path="<?php echo JComponentHelper::getParams('com_media')->get('image_path') ?>">
        ...
        <div class="uk-overflow-container">
            <ul class="uk-grid uk-grid-width-small-1-2 uk-grid-width-large-1-3 uk-grid-width-xlarge-1-4 uk-form" data-uk-grid-margin data-uk-grid-match="{target:'.uk-panel'}">
                ...
                <li ng-repeat="folder in media | filter: { type: 'folder' }">
                    <div ng-click="folder.selected = !folder.selected" class="uk-panel uk-panel-box uk-text-center uk-visible-hover" ng-class="folder.selected ? 'wk-selected':''">
                ...
                <li ng-repeat="file in media | filter: { type: 'file' }">
                    <div ng-click="file.selected = !file.selected" class="uk-panel uk-panel-box uk-text-center uk-visible-hover" ng-class="file.selected ? 'wk-selected':''">
                        <div class="uk-panel-teaser">
                            <div ng-if="file.image" class="wk-thumbnail" style="background-image: url('{{ file.href }}');"></div>
                            <div ng-if="!file.image" class="wk-thumbnail wk-thumbnail-file"></div>
                        </div>
