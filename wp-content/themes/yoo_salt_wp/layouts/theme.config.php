...
foreach ($blocks as $block) {
    $block_classes[$block] = $config->get("block.{$block}.block_padding") ? ' tm-block-padding' : '';
    $block_classes[$block] .= $config->get("block.{$block}.block_full_width") ? ' tm-block-full-width' : '';

    if ($block !== 'toolbar' && $block !== 'footer') {
        if ($this['config']->get("block.{$block}.gutter_collapse", false)) {

            $block_classes[$block] .= ' tm-block-collapse';

            array_key_exists($block, $grid_classes) ? $grid_classes[$block] .= ' uk-grid-collapse': $grid_classes[$block] = ' uk-grid-collapse';
        }
    }
}
