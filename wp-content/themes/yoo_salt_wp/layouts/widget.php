...
else if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer', 'footer-r', 'footer-l', 'offcanvas'))) {
	$panel = 'uk-panel';
}
...
// Set title
if (in_array($widget->position, array('headerbar', 'toolbar-r' ,'toolbar-l', 'footer-r', 'footer-l', 'footer'))) {
	$title = '';
} elseif ($title && !($widget->position == 'menu' || $widget->position == 'navbar-menu')) {
...
if ($widget->menu) {
	...
	} else if (in_array($widget->position, array('navbar-menu'))) {
		$renderer = 'navbar';
		$widget->nav_settings["modifier"] = "uk-hidden-small";
	} else if (in_array($widget->position, array('toolbar-l', 'toolbar-r', 'footer-r', 'footer-l', 'footer'))) {
		$renderer = 'subnav';
...
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
	echo $content;
} elseif ($widget->position == 'navbar-menu') {
	if ($widget->menu) {
		echo $content;
	} else {
		echo '
